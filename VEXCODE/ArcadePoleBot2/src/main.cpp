...
int main() {
  ...
  while (true){
    ...
    if (H.Axis4.value()!=0 || H.Axis3.value()!=0){
      xNew = H.Axis4.value() * cos(head*(M_PI/180)) - H.Axis3.value() * sin(head*(M_PI/180));
      yNew = H.Axis3.value() * cos(head*(M_PI/180)) + H.Axis4.value() * sin(head*(M_PI/180));
      if (xNew==0){
        Lturn = yNew/maxSpeed;
        Rturn = yNew/maxSpeed;
      }
    ...
    else if (H.Axis2.value()!=0 || H.Axis1.value()!=0){
