...
void Roller(double dist){
  for (motor m :base){
    m.resetRotation();
    m.spin(directionType::fwd,6,voltageUnits::volt);
  }
  vex::task::sleep(500);//wait for bot to start moving
  //double rotations[6] = {};
  /*while (true){
    for (motor m :base){
  ...
  intake.spin(directionType::rev,11,voltageUnits::volt);
  vex::task::sleep(500);
  //while (intake.rotation(rotationUnits::rev) <= -0.2){}
