...
motor RM = motor(PORT7, gearSetting::ratio18_1, false);
...
double wheelTrack = 29.5 * M_PI; //(distance between left and right wheels * pi)The distance a wheel will travel to spin the bot 360deg(cm)
...
bool reached(double initial, double target){
...
void Move(double left, double right){
  ...
  while (true){
    ...
    for (int i=0; i<=5; i++){
      ...
      Brain.Screen.print("  ");
      Brain.Screen.print(plan[i]);
      Brain.Screen.newLine();
      if (reached(base[i].rotation(rotationUnits::rev),plan[i])){
...
void Roller(double dist){
  for (int i=0;i<3;i++){
    base[i].resetRotation();
    base[i].spin(directionType::fwd,11,voltageUnits::volt);
  }
  vex::task::sleep(2000);//wait for bot to start moving
  //double rotations[6] = {};
  /*while (true){
    for (motor m :base){
      if (m.rotation(rotationUnits::rev)<=dist){
        break;
      }
    }
  }*/
  intake.resetRotation();
  intake.spin(directionType::fwd,11,voltageUnits::volt);
  vex::task::sleep(300);
  //while (intake.rotation(rotationUnits::rev) <= 0.7){}
...
void usercontrol(void) {
  ...
  while (1) {
    ...
    LM.spin(directionType::fwd, H.Axis3.value()/11.5, voltageUnits::volt);
    ...
    RM.spin(directionType::fwd, H.Axis2.value()/11.5, voltageUnits::volt);
