...
double wheelTrack2 = 12.1; //distance between left and right wheels(inch)
...
void Rotate2(float deg){
  ...
  RF.resetRotation();
  RB.resetRotation();
  while(CRad != TRad&&corrections <= 10){
...
void autonomous(void) {
  ...
  Rotate2(-45);
  for (motor m: base){
    m.spin(directionType::rev,8,voltageUnits::volt);
  }
  task::sleep(1000);
  for (motor m: base){
    m.stop(brakeType::hold);
